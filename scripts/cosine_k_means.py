...
# Normalize the TF-IDF matrix to use cosine similarity
# (cosine similarity between two vectors = dot product of their normalized versions)
normalized_tfidf = normalize(tfidf_matrix, norm='l2', axis=1)

# Perform K-means clustering with cosine similarity
# We normalize the vectors so that Euclidean distance on normalized vectors
# is equivalent to cosine similarity
n_clusters = 2
kmeans = KMeans(
    n_clusters=n_clusters,
    random_state=42,
    n_init=10  # Increased number of initializations for better results
)
cleaned_corpus['cluster'] = kmeans.fit_predict(normalized_tfidf)

# Perform LSA on the normalized TF-IDF matrix
lsa = TruncatedSVD(n_components=2, random_state=42)
doc_coords = lsa.fit_transform(normalized_tfidf)
